...
/**
 * Class that loads shader programs
 */
public class ShaderLoader
{
    ...
    /**
     * Creates singlton instance of ShaderLoader
     */
    ...
    /**
     * loads the vertex shader program and fragment shader
     * vertexShaderFileName - file name of vertex shader
     * fragmentShaderFileName - file name of the fragment shader
     */
    public bool Load(string vertexShaderFileName, string fragmentShaderFileName)
    {
        ...
        //set vertexHandle and fragmentHandle to IDs of VertexShader and FragmentShader objects
        vertexHandle = GL.CreateShader(ShaderType.VertexShader);
        fragmentHandle = GL.CreateShader(ShaderType.FragmentShader);

        //if VertexShader ID or FragmentShader ID are 0 then one or both shaders weren't created
        ...
        //load and compile the VertexShader, return false if load and compile fails
        if (!LoadAndCompileShader(vertexShaderFileName, vertexHandle))
            return false;

        //load and compile the FragmentShader, return false if load and comile fails
        ...
        //if ShaderProgram ID = 0, then CreateProgram failed
        if (programHandle == 0)
        {
            Unload();
    ...
    //delete the ShaderProgram and Shaders
    public void Unload()
    {
        if (programHandle != 0)
        {
    ...
    //retrun the most recent error message
    public string LastLoadError
    {
        get { return loadError; }
    }

    //read and compile shader from GLSL, return false if read and compile fails
